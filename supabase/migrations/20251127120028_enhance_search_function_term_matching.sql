-- ============================================
-- ENHANCED SEARCH FUNCTION WITH TERM MATCHING
-- Updates search_road_signs to match individual query terms
-- ============================================
...
BEGIN
  ...
  WITH candidates AS (
    SELECT
      ...
      rsi.artist_name,
      rsi.usage_count,
      -- Compute score with individual term matching
      (
        ...
        (CASE WHEN mapped_sign IS NOT NULL AND rsi.file_name ILIKE '%' || mapped_sign || '%' THEN 95 ELSE 0 END) +
        (CASE WHEN mapped_sign IS NOT NULL AND rsi.filename_slug ILIKE '%' || mapped_sign || '%' THEN 95 ELSE 0 END) +
        (CASE WHEN mapped_sign IS NOT NULL AND rsi.wikimedia_file_name ILIKE '%' || mapped_sign || '%' THEN 90 ELSE 0 END) +
        ...
        -- Tier 2/3 Enhancement: Individual term matches for filename/name fields
    ...
    WHERE rsi.image_source = 'wikimedia_commons'
      ...
      AND (
        ...
        OR
        -- Tier 2 matches (individual terms)
        EXISTS (
          SELECT 1 FROM unnest(query_terms) AS q_term
          WHERE rsi.file_name ILIKE '%' || q_term || '%'
             OR rsi.filename_slug ILIKE '%' || q_term || '%'
        ...
        OR
        -- Tier 3 matches (individual terms)
        EXISTS (
          SELECT 1 FROM unnest(query_terms) AS q_term
          WHERE rsi.sign_name_en ILIKE '%' || q_term || '%'
             OR rsi.sign_name_jp LIKE '%' || q_term || '%'
  ...
  SELECT
    candidates.id, candidates.storage_url, candidates.file_name, candidates.filename_slug, candidates.wikimedia_file_name, candidates.sign_number,
    candidates.sign_name_en, candidates.sign_name_jp, candidates.sign_category, candidates.tags, candidates.attribution_text, candidates.license_info,
    candidates.wikimedia_page_url, candidates.artist_name, candidates.score_calc::INTEGER AS score
  FROM candidates
  WHERE score_calc > 0
  ORDER BY score_calc DESC, usage_count DESC NULLS LAST
  LIMIT 25;
END;
$$;
...
COMMENT ON FUNCTION public.search_road_signs IS 'Enhanced 6-tier search with individual term matching. Searches by: 1) Sign code number, 2) Filename (full query + individual terms), 3) Sign names (full query + individual terms), 4) Tags, 5) Category, 6) Usage quality. Returns ranked results with relevance scores.';
