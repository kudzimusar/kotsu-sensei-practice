...
async function getDatabaseStats() {
  try {
    ...
    return {
      ...
      lastUploadTime: lastUpload?.updated_at || null,
    };
  } catch (error: any) {
    return { total: 0, recent: 0, categories: {}, lastUploadTime: null, error: error.message };
  }
}

function checkProcessRunning() {
  try {
    const mainResult = execSync('ps aux | grep -c "[d]ownload-all-categories"', { encoding: 'utf-8' });
    const mainCount = parseInt(mainResult.trim()) || 0;
...
async function displayProgress() {
  ...
  if (dbStats.lastUploadTime) {
    const lastUpload = new Date(dbStats.lastUploadTime);
    const secondsAgo = Math.floor((Date.now() - lastUpload.getTime()) / 1000);
    console.log(`   Last upload: ${secondsAgo} seconds ago`);
  }
...
process.on('SIGINT', () => {
  ...
  console.log('   Check progress with: cat all-categories-download-report.json');
  process.exit(0);
});
